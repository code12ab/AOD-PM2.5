...
input_path = 'D:\\毕业论文D盘数据\\D盘根目录数据\\雨雪+2018_new_pm_aod_interpolate.xlsx'  # \\雨雪+2018_new_pm_aod_interpolate.xlsx'
input_path2 = 'D:\\毕业论文程序\\08到17年\\输入特征_十年\\08-17.xlsx'

data_all = pd.read_excel(input_path, index_col='日期')
data_all2 = pd.read_excel(input_path2, index_col='日期')
...
data_ts_df2= data_all2[['tm_mon', 'tm_mday',
                       'tm_wday', 'tm_yday', 'tm_week', 'id']]
...
data_dummies = pd.concat([data_get_dummies1,
                          ...
                          data_ts_df[['id']]],
                         axis=1)
#for xxx in data_get_dummies3.columns:
#    print(xxx)
for ccc2 in data_ts_df2.columns:
    data_ts_df2[ccc2] = data_ts_df2[ccc2].map(lambda x: str(x))
data_get_dummies12 = pd.get_dummies(data_ts_df2[['tm_mon']], drop_first=True)
data_get_dummies32 = pd.get_dummies(data_ts_df2[['id']], drop_first=True)
for x in range(12,152):
        if x != 148 and x!= 147 and x!=146:
            data_get_dummies32['id_%s' % x] = int(0)
data_dummies2 = pd.concat([data_get_dummies12,
                          data_get_dummies32,
                          data_ts_df2[['tm_mon']],
                          data_ts_df2[['id']]],
                         axis=1)
...
data_std2 = copy.deepcopy(data_to_std2)
mean_pm2 = data_std2['PM25'].mean()
std_pm2 = data_std2['PM25'].std()
...
for col2 in data_std2:
    mean2 = data_std2[col2].mean()
    std2 = data_std2[col2].std()
    data_std2[col2] = data_std2[col2].map(lambda x:(x-mean2)/std2)
# 标准化后的数据矩阵
data_out = pd.concat([data_dummies, data_std], join='outer', axis=1)
data_out22 = pd.concat([data_dummies2, data_std2], join='outer', axis=1)
# 标准化前的数据矩阵
data_out2 = pd.concat([data_dummies, data_to_std], join='outer', axis=1)  # 标准化前的真实值
data_out222 = pd.concat([data_dummies2, data_to_std2], join='outer', axis=1)  # 标准化前的真实值
# 打乱
...
slice1 = random.sample(idlist, 12)  # 从list中随机获取3个元素，作为一个片断返回
...
data_test2 = data_out22[data_out22["tm_mon"].isin(slice1)]

# AOD
data_aod_test = data_test[['AOD_0']]
data_aod_test2 = data_test2[['AOD_0']]
...
data_time_test2 = data_test2[['tm_mon_10',
                            'tm_mon_11',
                            'tm_mon_12',
                            'tm_mon_2',
                            'tm_mon_3',
...
data_station_test2 = data_test2[data_get_dummies32.columns]
# 时滞
...
data_t1_test2 = data_test2[['AOD_0_T1',
                          'cloudCover_T1',
                          'dewPoint_T1',
                          'humidity_T1',
                          'sunTime_T1',
...
data_ndvi_test2 = data_test2[['NDVI_0']]
...
data_aods_test = data_test[['AOD_1',
                            ...
                            'AOD_15',
                            'AOD_16']]
data_aods_test2 = data_test2[['AOD_1',
                            'AOD_2',
                            'AOD_3',
                            'AOD_4',
                            'AOD_5',
...
# 格式转换
data_sky_train = np.array(data_sky_test)
data_t1_train = np.array(data_t1_test)
data_ndvi_train = np.array(data_ndvi_test)
data_time_train = np.array(data_time_test)
data_station_train = np.array(data_station_test)
data_aods_train = np.array(data_aods_test)
data_aod_train = np.array(data_aod_test)  # 这个test 是 18年
...
# 运行
# 输入顺序： 气象 时滞 NDVI 时间 空间 AODs AOD
# 这里的train实际是18年的，即test数据，懒得改名字了
...
# 预测
res = model_last.predict([data_sky_test2,
                          data_t1_test2,
                          data_ndvi_test2,
                          data_time_test2,
                          data_station_test2,
                          ...
                          data_aod_test2])
# 还原，反标准化  输出的是08-17
res2 = [float((j * std_pm2) + mean_pm2) for j in res]
res2 = pd.DataFrame(res2, index=data_pm_test2.index, columns=['PM25'])
res2['id'] = data_dummies2['id']
res2.to_excel('D:\\毕业论文程序\\08到17年\\输出特征\\result08-17.xlsx')
