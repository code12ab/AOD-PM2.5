...
for item in JCZ:
    ...
    # 重设列名
    #aod_outcome_list_result.columns = ['日期', '监测站', "AOD值"]
    # 同日期，多文件情况下的均值处理
    #aod_outcome_list_result = aod_outcome_list_result.groupby(['日期', "监测站"]).mean()
    # 美化group by均值计算后的数据框格式
    #aod_outcome_list_result = pd.Series(aod_outcome_list_result["AOD值"])  # AOD值按分组计算的结果
    aod_outcome_list_result.to_excel(output_file_path+"%s.xlsx" % item[2])  # 完整结果存入excel
