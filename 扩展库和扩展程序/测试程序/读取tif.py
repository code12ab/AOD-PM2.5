...
for tif in list_tif:
    ...
    if in_ds is None:
        ...
        a = in_ds.GetRasterBand(1)
        b = a.GetRasterColorInterpretation
        moudel_list = dir(a)
        print(b)
        print(moudel_list)

        '''
        red = in_ds.GetRasterBand(1).GetDataset().ReadAsArray() * 0.0001
        nir = in_ds.GetRasterBand(2).GetDataset().ReadAsArray() * 0.0001
