...
data_ewm = pd.read_excel(mean_output_file_path, sheet_name="ewm")
data_IDW = pd.read_excel(mean_output_file_path, sheet_name="IDW")
data_Iterative = pd.read_excel(mean_output_file_path, sheet_name="Iterative")
...
for area_numb in range(0, 17):
    ...
    for loc in range(len(data_aod.index)):
        # 四种方法均不缺失
        if pd.notnull(data_aod["KNN"][loc]) and pd.notnull(data_aod["ewm"][loc]) and pd.notnull(data_aod["IDW"][loc]) and pd.notnull(data_aod["Iterative"][loc]):
    ...
    connect_data = pd.merge(data_aod, value_weight_list, left_index=True, right_index=True)
    connect_data = connect_data.drop(columns=["KNN", "ewm", "IDW", "Iterative"])
