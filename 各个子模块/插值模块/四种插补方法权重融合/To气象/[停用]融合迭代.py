...
mean_output_file_path = "D:\\毕业论文程序\\气象数据\\插值模块\\Merge\\2018\\"
res_output_path = "D:\\毕业论文程序\\气象数据\\插值模块\\Res\\2018_不补全\\"
...
for input_file_name in input_file_names:
    ...
    if str(
        data_KNN.set_index('日期').stack().max()) == "nan" and str(
        data_ewm.set_index('日期').stack().max()) == "nan" and str(
            data_IDW.set_index('日期').stack().max()) == "nan" and str(
                data_Iterative.set_index('日期').stack().max()) == "nan":
    ...
    for darksky_weather in [
        ...
        'apparentTemperature',
            'temperature']:
        d1 = data_KNN[["日期", darksky_weather]]
        d2 = data_ewm[["日期", darksky_weather]]
        ...
        d3 = data_IDW[["日期", darksky_weather]]
        d4 = data_Iterative[["日期", darksky_weather]]
        data_Station = pd.merge(d3, d4, how='right', on=["日期"])
        data_NDVI = pd.merge(
                data_Time,
                data_Station,
                how='right',
        ...
        if len(data_NDVI_to_weight.index) > 0:
            ...
            if len(w_def.index) == 0:
                w_def = pd.DataFrame([np.nan, np.nan, np.nan, np.nan],
                                     index=[
                    'KNN',
                    'ewm',
                    'IDW',
            ...
            # print(w_def, w_null)
            if str(w_def) == 'None':
                w123 = pd.DataFrame([['KNN', np.nan, np.nan],
                                     ['ewm', np.nan, np.nan],
                                     ['IDW', np.nan, np.nan],
                                     ['Iterative', np.nan, np.nan]],
                                     columns=['method','weight_null','weight_def'])
        ...
        value_weight_list.columns = [darksky_weather]
