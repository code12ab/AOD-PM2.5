...
# 标准化前的数据矩阵
data_out2 = pd.concat([data_dummies, data_to_std],
                      join='outer', axis=1)  # 标准化前的真实值
...
for t_numb in range(0, 20):
    ...
    x1 = layers.Dense(
        12, activation=keras.layers.advanced_activations.LeakyReLU(
    ...
    x3 = layers.Dense(
        24, activation=keras.layers.advanced_activations.LeakyReLU(
            alpha=0.5))(inputTime)
    x3_residual_connection1 = Dense(
        24, activation=keras.layers.LeakyReLU(
            alpha=0.2))(x3)
    x3_residual_connection2 = Dense(
        24, activation=keras.layers.advanced_activations.ELU(
            alpha=1.0))(x3_residual_connection1)
    x3_residual_output = add([x3, x3_residual_connection2])
    x3 = layers.Dense(
        12, activation=keras.layers.advanced_activations.LeakyReLU(
            alpha=0.5))(x3_residual_output)
    ...
    x4_residual_connection1 = Dense(
        24, activation=keras.layers.LeakyReLU(
            alpha=0.2))(x4)
    x4_residual_connection2 = Dense(
        24, activation=keras.layers.advanced_activations.ELU(
            alpha=1.0))(x4_residual_connection1)
    x4_residual_output = add([x4, x4_residual_connection2])
    x4 = layers.Dense(
        12, activation=keras.layers.advanced_activations.LeakyReLU(
            alpha=0.5))(x4_residual_output)
    ...
    x5 = layers.Dense(
        24, activation=keras.layers.advanced_activations.LeakyReLU(
            alpha=0.5))(inputT1)
    x5_residual_connection1 = Dense(
        24, activation=keras.layers.LeakyReLU(
            alpha=0.2))(x5)
    x5_residual_connection2 = Dense(
        24, activation=keras.layers.advanced_activations.ELU(
            alpha=1.0))(x5_residual_connection1)
    x5_residual_output = add([x5, x5_residual_connection2])
    x5 = layers.Dense(
        12, activation=keras.layers.advanced_activations.LeakyReLU(
            alpha=0.5))(x5_residual_output)
    ...
    x6 = layers.Dense(
        24, activation=keras.layers.advanced_activations.LeakyReLU(
            alpha=0.5))(inputNDVI)
    x6_residual_connection1 = Dense(
        24, activation=keras.layers.LeakyReLU(
            alpha=0.2))(x6)
    x6_residual_connection2 = Dense(
        24, activation=keras.layers.advanced_activations.ELU(
            alpha=1.0))(x6_residual_connection1)
    x6_residual_output = add([x6, x6_residual_connection2])
    x6 = layers.Dense(
        12, activation=keras.layers.advanced_activations.LeakyReLU(
            alpha=0.5))(x6_residual_output)
    ...
    x7 = layers.Dense(
        24, activation=keras.layers.advanced_activations.LeakyReLU(
            alpha=0.5))(inputAODs)
    x7_residual_connection1 = Dense(
        24, activation=keras.layers.LeakyReLU(
            alpha=0.2))(x7)
    x7_residual_connection2 = Dense(
        24, activation=keras.layers.advanced_activations.ELU(
            alpha=1.0))(x7_residual_connection1)
    x7_residual_output = add([x7, x7_residual_connection2])
    x7 = layers.Dense(
        12, activation=keras.layers.advanced_activations.LeakyReLU(
            alpha=0.5))(x7_residual_output)
    ...
    res_residual_connection2 = Dense(
        8, activation=keras.layers.LeakyReLU(
            alpha=0.2))(res_residual_connection1)
    ...
    z = keras.layers.LSTM(
        ...
        stateful=False,
        unroll=False)(z)
    z = Dense(1, activation=keras.layers.LeakyReLU(alpha=0.2),
    ...
    # print(datares)
    datares.PM25 = datares.PM25.map(lambda x: abs(x))
    data_predt = pd.concat([datares, data_test2.PM25],
                           axis=1)  # 标准化后真值变化了 需要修改
