...
for item in JCZ:
    ...
    # 距离计算，提取监测站半径为r范围内的AOD值
        aod_list = []
        for column in range(longitude.shape[1]):  # 列
            for row in range(longitude.shape[0]):  # 行
                d = geo_distance(longitude[column][row], latitude[column][row], item[0], item[1])
                if (d > 0) and (d < r) and aod[column][row] > 0:
                    aod_list.append(aod[column][row])
