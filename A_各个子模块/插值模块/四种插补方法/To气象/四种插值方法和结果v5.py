...
def get4method(xx152):
                ...
                if pd.isnull(input_data[darksky_weather][indx]):  # 开始循环
                    for item_idw in JCZ_info["监测站"]:  # 获取距离,定义权重
                        if item_idw != name:
                            ...
                            if dis_1 <= 50000:
                                data_to_add_in_1 = pd.read_excel(input_file_path_darksky_weather + item_idw + ".xlsx")
                                data_to_add_in_1 = data_to_add_in_1.set_index("日期")  # 需要日期为索引,方便下面添加
                                if indx in data_to_add_in_1.index and pd.notnull(data_to_add_in_1[darksky_weather][indx]):
                    ...
                    for item_idw_2 in JCZ_info["监测站"]:  # 分配权重
                        if item_idw_2 != name:
                            ...
                            dis_1 = geo_distance(lng1, lat1, lng2, lat2)  # 两站地理距离
                            if dis_1 <= 50000:
                                data_to_add_in = pd.read_excel(input_file_path_darksky_weather + item_idw_2 + ".xlsx")
                                data_to_add_in = data_to_add_in.set_index("日期")  # 需要日期为索引,方便下面添加
                                if indx in data_to_add_in.index and pd.notnull(data_to_add_in[darksky_weather][indx]):
                                    res = (dis_1 / weight_sum) * data_to_add_in[darksky_weather][indx]
                                    res_list.append(res)
                                    # print("已添加单元格插值:", res)
    ...
    for input_file_name in jcz_152["监测站名称_152"]:
        ...
        try:
                ...
                for item in JCZ_info["监测站"]:  # 不同于气溶胶插值方法
                    if item != name:
                        ...
                        if dis_2 <= 50000:  # 合并距离内的临近监测站
                            ...
                            data_to_Iterative_concat = data_to_add_in_to_Iterative[darksky_weather_Iterative]
            ...
            data_darksky_weather_ewm = data_darksky_weather_ewm.set_index(data_darksky_weather.index)
