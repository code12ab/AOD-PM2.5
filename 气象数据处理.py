...
for key in data.columns:
    data["%s" % key] = data["%s" % key].fillna(method='ffill')  # 相似
for key in data.columns:
    data["%s" % key] = data["%s" % key].interpolate()  # 线性填充

for key in data.columns:
    data["%s" % key] = data["%s" % key].fillna(method='bfill')  # 后
...
data_combine.to_excel(output_path+"combine\\%s_combine.xlsx" % file_name)
