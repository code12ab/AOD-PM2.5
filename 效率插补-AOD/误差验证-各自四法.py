# -*- coding: utf-8 -*-
# 作者: xcl
# 时间: 2019/8/7 18:54

# 库
import pandas as pd
import numpy as np
import math
from numpy import array
import os

res_output_path = "D:\\毕业论文程序\\气溶胶光学厚度\\插值模块\\Res\\2018插补效率\\"  # 制造空值后的插补结果
null_output_path = "D:\\毕业论文程序\\气溶胶光学厚度\\插值模块\\制造的缺失值\\"  # 制造出来的空值的，真实值。
input_file_path_Aqua = "D:\\毕业论文程序\\气溶胶光学厚度\\空间转换模块\\Aqua插补效率\\2018\\"  # 插补前数据
input_file_path_Terra = "D:\\毕业论文程序\\气溶胶光学厚度\\空间转换模块\\Terra插补效率\\2018\\"
mean_output_path = "D:\\毕业论文程序\\气溶胶光学厚度\\插值模块\\Mean\\2018插补效率\\"  # 制造空值后的插补结果

input_file_names = os.listdir(res_output_path)  # 文件名列表
for input_file_name in input_file_names:
    # print(input_file_name)
    data_res1 = pd.read_excel(mean_output_path + input_file_name, sheet_name='KNN')
    data_res2 = pd.read_excel(mean_output_path + input_file_name, sheet_name='ewm')
    data_res3 = pd.read_excel(mean_output_path + input_file_name, sheet_name='IDW')
    data_res4 = pd.read_excel(mean_output_path + input_file_name, sheet_name='Iterative')
    data_null_Aqua = pd.read_excel(null_output_path + "Aqua\\" + input_file_name, index_col='Unnamed: 0')
    data_null_Terra = pd.read_excel(null_output_path + "Terra\\" + input_file_name, index_col='Unnamed: 0')  # 第一列空列，处理一下
    data_origin_Terra = pd.read_excel(input_file_path_Terra + input_file_name)  # 插补前
    data_origin_Aqua = pd.read_excel(input_file_path_Aqua + input_file_name)
    data_res1['日期'] = data_res1['日期'].map(lambda x: x.replace(" ",""))
    data_res2['日期'] = data_res2['日期'].map(lambda x: x.replace(" ",""))
    data_res3['日期'] = data_res3['日期'].map(lambda x: x.replace(" ",""))
    data_res4['日期'] = data_res4['日期'].map(lambda x: x.replace(" ",""))

    data_origin_Terra['日期'] = data_origin_Terra['日期'].map(lambda x: x.replace(" ",""))
    data_origin_Aqua['日期'] = data_origin_Aqua['日期'].map(lambda x: x.replace(" ", ""))
    # print(data_res)
    # print(data_null.isnull())
    error1 = list()
    error2 = list()
    error3 = list()
    error4 = list()


    for columname in data_null_Aqua.columns:
        if columname != "日期":
            if columname != "监测站":
                # loc 是某列为空的行坐标，这里由于前一个步骤中被转置了，
                # loc 对应的是AOD0到AOD16
                # print(columname)
                loc_t = data_null_Aqua[columname][data_null_Aqua[columname].isnull().values == False].index.tolist()
                # print(loc_t)

                for numb1 in loc_t:
                    if numb1 == 0:  # 此时为 aod_0
                        # 原真实值。 Colu列→源数据第几行；numb（loc-t）第几行即AODx
                        c_null = data_null_Aqua[columname][numb1].replace("(","").replace(")","").replace("'","").replace(" ","").split(",")
                        c_null = list(c_null)  # 转化格式完毕
                        # print(c_origin[1])
                        # print(c_origin[1].__class__)
                        # 先 日期 ， 后 aod 变量
                        c_true1 = data_origin_Terra[data_origin_Terra['日期'] == c_null[1]][c_null[0]].values[0]  # 提出数值
                        c_chabu1 = data_res1[data_res1['日期'] == c_null[1]][c_null[0]].values[0]  # 这个相当于预测值
                        c_chabu2 = data_res2[data_res2['日期'] == c_null[1]][c_null[0]].values[0]  # 这个相当于预测值
                        c_chabu3 = data_res3[data_res3['日期'] == c_null[1]][c_null[0]].values[0]  # 这个相当于预测值
                        c_chabu4 = data_res4[data_res4['日期'] == c_null[1]][c_null[0]].values[0]  # 这个相当于预测值

                        if c_true1 > 0:
                            c_res1 = (float(c_true1) + float(c_null[2]))*0.5  # T + A 均值即为 观测值终值 相当于真实值
                            e1 = abs(c_chabu1 - c_res1)/c_res1
                            e2 = abs(c_chabu2 - c_res1)/c_res1
                            e3 = abs(c_chabu3 - c_res1)/c_res1
                            e4 = abs(c_chabu4 - c_res1)/c_res1
                            error1.append(e1)
                            error2.append(e2)
                            error3.append(e3)
                            error4.append(e4)

    for columname in data_null_Terra.columns:
        if columname != "日期":
            if columname != "监测站":
                # loc 是某列为空的行坐标，这里由于前一个步骤中被转置了，
                # loc 对应的是AOD0到AOD16
                # print(columname)
                loc_t = data_null_Terra[columname][data_null_Terra[columname].isnull().values == False].index.tolist()
                # print(loc_t)

                for numb1 in loc_t:
                    if numb1 == 0:  # 此时为 aod_0
                        # 原真实值。 Colu列→源数据第几行；numb（loc-t）第几行即AODx
                        c_null = data_null_Terra[columname][numb1].replace("(","").replace(")","").replace("'","").replace(" ","").split(",")
                        c_null = list(c_null)  # 转化格式完毕
                        # print(c_origin[1])
                        # print(c_origin[1].__class__)
                        # 先 日期 ， 后 aod 变量
                        # print(data_origin_Aqua[data_origin_Aqua['日期'] == c_null[1]][c_null[0]].values[0])  # 提出数值
                        c_true1 = data_origin_Aqua[data_origin_Aqua['日期'] == c_null[1]][c_null[0]].values[0]  # 提出数值
                        c_chabu1 = data_res1[data_res1['日期'] == c_null[1]][c_null[0]].values[0]  # 这个相当于预测值
                        c_chabu2 = data_res2[data_res2['日期'] == c_null[1]][c_null[0]].values[0]  # 这个相当于预测值
                        c_chabu3 = data_res3[data_res3['日期'] == c_null[1]][c_null[0]].values[0]  # 这个相当于预测值
                        c_chabu4 = data_res4[data_res4['日期'] == c_null[1]][c_null[0]].values[0]  # 这个相当于预测值
                        if c_true1 > 0:
                            c_res1 = (float(c_true1) + float(c_null[2]))*0.5  # T + A 均值即为 观测值终值 相当于真实值
                            e1 = abs(c_chabu1 - c_res1)/c_res1
                            e2 = abs(c_chabu2 - c_res1)/c_res1
                            e3 = abs(c_chabu3 - c_res1)/c_res1
                            e4 = abs(c_chabu4 - c_res1)/c_res1
                            error1.append(e1)
                            error2.append(e2)
                            error3.append(e3)
                            error4.append(e4)
print(np.average(error1),np.average(error2),np.average(error3),np.average(error4))
print(np.std(error1),np.std(error2),np.std(error3),np.std(error4))

    # print(data_null_Terra.columns)
    # print(data_res.head(3))
    # print(data_res['日期'][1].__class__)
    # print(data_res['日期'][1])
    # print(data_res[data_res['日期'] == '2018-01-01 ']['AOD_0'])  # 日期后面多了个空格
    # lng2 = JCZ_info[JCZ_info["监测站"] == item_idw]["经度"]

