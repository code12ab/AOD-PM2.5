...
out_path = 'D:\\毕业论文程序\\非线性检验\\相关系数\\各自\\'
out_path2 = 'D:\\毕业论文程序\\非线性检验\\相关系数\\汇总\\'
...
for name in file_name_list:
    ...
    for name2 in file_name_list:
        if name2 != name:
            ...
            corr, pval = pearsonr(data0['PM25'], data1['PM25'])
            # print("P", pval, "PERA" , corr)
            if pd.isna(pval):
                # print(pval)
                pval = 0.000
            if pval < 0.05:
    ...
    # print(res.head(3))
    res.columns = ['监测站', "相关系数", "地理距离"]  # 重命名

    res = res.fillna(method='pad')  # 前一个
    res = res.fillna(method='backfill')  # 后一个
